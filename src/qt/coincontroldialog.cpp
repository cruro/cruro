...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of cruries so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    // how many cruries the estimated fee can vary per byte we guess wrong
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show Cruro address. In tree mode, address is not shown again for direct wallet address outputs
