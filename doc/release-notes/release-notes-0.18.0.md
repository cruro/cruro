...
recommended to use Cruro Core on unsupported systems.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Cruro Core
...
`~bitcoin/.bitcoin`. This change makes Cruro Core more consistent with
...
- A new document introduces Cruro Core's BIP174 [Partially-Signed
  Cruro Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  ...
  Cruro Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Cruro Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Cruro Core, subsequently importing
...
This section describes planned changes to Cruro Core that may affect
other Cruro software and services.

- Since version 0.16.0, Cruro Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Cruro Core 0.20 (expected about
  a year after 0.18), Cruro Core will default to native segwit
  ...
  to bech32 addresses, and if the Cruro Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Cruro Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Cruro Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Cruro Core
...
- A sub-project of Cruro Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Cruro Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Cruro Core's own implementation, although entropy
  gathered by Cruro Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Cruro Core a
...
- On macOS, Cruro Core now opts out of application CPU throttling
...
- #11526 Visual Studio build configuration for Cruro Core (sipsorcery)
...
- #12775 Integration of property based testing into Cruro Core (Christewart)
...
- #15802 mention creating application support Cruro folder on OSX (JimmyMow)
