...
In addition to that, with 0.12.x there have been varied reports of Cruro Core
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Cruro wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Cruro block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Cruro addresses (both P2PKH addresses
...
- **More efficient almost-full-node security** crury Nakamoto's original
  Cruro paper describes a method for allowing newly-started full nodes to
  ...
  note that Cruro Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Cruro users to individually opt-in to almost any change in the Bitcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Cruro Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Cruro community members are studying several other improvements
...
Since Cruro Core 0.10.0, nodes have defaulted to only relaying and
